...
func TestParseArgs_HelpLong(t *testing.T) {
	verbose, version, showHelp, printEnv, model, upstream, logFile, profile, _, _, _, opencodeArgs := parseArgs([]string{"--help"})
	if !showHelp {
...
func TestParseArgs_LogFile(t *testing.T) {
	_, _, _, _, _, _, logFile, _, _, _, _, _ := parseArgs([]string{"--log-file", "/tmp/test.log"})
...
func TestParseArgs_Upstream(t *testing.T) {
	_, _, _, _, _, upstream, _, _, _, _, _, _ := parseArgs([]string{"--upstream", "https://gw.example.com/openai/v1"})
...
func TestParseArgs_Model(t *testing.T) {
	_, _, _, _, model, _, _, _, _, _, _, _ := parseArgs([]string{"--model", "gpt-4o"})
	if model != "gpt-4o" {
...
func TestParseArgs_UnknownFlagPassthrough(t *testing.T) {
	_, _, _, _, _, _, _, _, _, _, _, opencodeArgs := parseArgs([]string{"--unknown"})
...
func TestParseArgs_EmptyArgs(t *testing.T) {
	verbose, version, showHelp, printEnv, model, upstream, logFile, profile, _, _, _, opencodeArgs := parseArgs([]string{})
...
func TestParseArgs_Mixed(t *testing.T) {
	verbose, _, showHelp, _, _, upstream, _, _, _, _, _, _ := parseArgs([]string{"--verbose", "--upstream", "https://gw.example.com", "--help"})
	if !showHelp {
...
func TestParseArgs_Separator(t *testing.T) {
	verbose, _, _, _, _, _, _, _, _, _, _, opencodeArgs := parseArgs([]string{"--verbose", "--", "--unknown", "arg1"})
	if !verbose {
...
func TestParseArgs_PassthroughArgs(t *testing.T) {
	_, _, _, _, _, _, _, _, _, _, _, opencodeArgs := parseArgs([]string{"prompt text", "--some-flag", "value"})
...
func TestParseArgs_Table(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			verbose, version, showHelp, printEnv, model, upstream, logFile, profile, _, _, _, opencodeArgs := parseArgs(tc.args)
...
func TestParseArgs_Profile(t *testing.T) {
	_, _, _, _, _, _, _, profile, _, _, _, _ := parseArgs([]string{"--profile", "aidev"})
	if profile != "aidev" {
...
func TestParseArgs_ProfileEquals(t *testing.T) {
	_, _, _, _, _, _, _, profile, _, _, _, _ := parseArgs([]string{"--profile=production"})
...
func TestHandleHelp_AllFlagsPresent(t *testing.T) {
	...
	flags := []string{"--profile", "--upstream", "--verbose", "-v", "--log-file", "--model", "--version", "--help"}
